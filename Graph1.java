...
this.numVertices = numVertices; adjLists = new ArrayList<>();

for (int i = 0; i < numVertices; i++) { adjLists.add(new ArrayList<>());

}

}

public void addEdge(int src, int dest, int weight) { adjLists.get(src).add(new Edge(dest, weight)); }

public void greedySearch(int src) {

int[] distances = new int[numVertices]; Arrays.fill(distances, Integer.MAX_VALUE); distances [src] = 0;
...
if (!visited[i] && distances [i] < minDistance) { minDistance = distances[i];
...
public static void main(String[] args) { Graph1 graph = new Graph1(6);
